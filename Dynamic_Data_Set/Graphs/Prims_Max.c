...
float Prims_Max(float H[] , struct listnode *list[] , int s , int n){
  ...
  for(int i = 0; i<n && H[0]>INT_MIN; i++){
    ...
    while(node!=NULL){
      ...
      if(V[v]==0 && H[B[v]] < H[B[u]] + node->weight){
        IncreaseKey(H , A , B , B[v] , H[B[u]] + node->weight);
        phi[v] = u;
      }
...
int main(){
    ...
    float H[LARGE];
    float ans = Prims_Max(H , list , s , n); 
